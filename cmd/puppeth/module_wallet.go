...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gjart --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec gjart --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --ethstats \'{{.Ethstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh
...
services:
  wallet:
    ...
    ports:
      ...
      - "{{.RPCPort}}:7864"{{if not .VHost}}
      - "{{.WebPort}}:80"{{end}}
    volumes:
      - {{.Datadir}}:/root/.jartdium
    environment:
...
func checkWallet(client *sshClient, network string) (*walletInfos, error) {
	...
	rpcPort := infos.portmap["7864/tcp"]
