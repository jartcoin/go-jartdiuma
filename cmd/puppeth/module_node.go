...
{{end}}
RUN \
  echo 'gjart --cache 512 init /genesis.json' > gjart.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.jartdium/keystore/ && cp /signer.json /root/.jartdium/keystore/' >> gjart.sh && \{{end}}
	echo $'exec gjart --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.etherbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> gjart.sh

ENTRYPOINT ["/bin/sh", "gjart.sh"]
`
...
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.jartdium{{if .Ethashdir}}
      - {{.Ethashdir}}:/root/.jarthash{{end}}
    environment:
      - PORT={{.Port}}/tcp
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.jartdium"],
		ethashdir:  infos.volumes["/root/.jarthash"],
		port:       port,
