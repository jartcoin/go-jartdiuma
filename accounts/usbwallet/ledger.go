...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "Jartdium app in browser mode", w.failure
	}
	if w.offline() {
		return "Jartdium app offline", w.failure
	}
	return fmt.Sprintf("Jartdium app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}
