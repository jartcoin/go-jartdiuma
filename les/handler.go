...
import (
	...
	"github.com/jartcoin/go-jartdiuma/core/state"
	...
	"github.com/jartcoin/go-jartdiuma/light"
	...
	"github.com/jartcoin/go-jartdiuma/trie"
)
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light Jartdium protocol")
	...
	log.Info("Light Jartdium protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Light Jartdium peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, genesis.Hash(), pm.server); err != nil {
		p.Log().Debug("Light Jartdium handshake failed", "err", err)
	...
	if err := pm.peers.Register(p); err != nil {
		clientErrorMeter.Mark(1)
		p.Log().Error("Light Jartdium peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light Jartdium message handling failed", "err", err)
			if p.fcServer != nil {
				p.fcServer.DumpLogs()
			}
			return err
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light Jartdium message arrived", "code", msg.Code, "bytes", msg.Size)
